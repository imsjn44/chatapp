...
export const createNewChat = TryCatch(async (req: Request, res) => {
...
export const getAllChats = TryCatch(async (req: Request, res) => {
  const userId = req.user?._id;
  if (!userId) {
    res.status(400).json({
...
export const sendMessage = TryCatch(async (req: Request, res) => {
...
export const getMessagesByChat = TryCatch(async (req: Request, res) => {
  const userId = req.user?._id;
    ...
    if (messagesToMarkSeen.length > 0) {
      ...
      if (otherUserSocketId) {
        io.to(otherUserSocketId).emit("messagesSeen", {
          chatId: chatId,
          seenBy: userId,
          meesagesId: messagesToMarkSeen.map((msg) => msg._id),
        });
      }
    }
    res.json({
